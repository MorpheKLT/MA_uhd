...
{
    std::vector<samp_type> latest_buffer;
    std::mutex mtx;
    ...
    py::object py_module;
    {
        ...
        std::cout<<"start loading module"<<std::endl;
        py::module sys = py::module::import("sys");
        sys.attr("path").attr("insert")(0, "/home/wydl/Projects/uhd/host/examples");
        sys.attr("stdout") = sys.attr("__stdout__");
        py_module = py::module::import("realtime_infer.infer");
        py_module.attr("model") = py_module.attr("load_model_main")();
        std::cout<<"finish loading module"<<std::endl;
    }
    std::thread py_thread([&]() {
        std::vector<samp_type>* data_copy;
        while (!stop_signal_called && !done) {
            ...
            // std::cout<<"copy data"<<std::endl;
            {
                ...
                data_copy = new std::vector<samp_type>(latest_buffer);
                has_new_data = false;
            }
            // std::cout<<"transform data"<<std::endl;
            {
                ...
                auto capsule = py::capsule(data_copy, [](void *data_copy) { delete reinterpret_cast<std::vector<samp_type>*>(data_copy); });
                py_module.attr("infer")(py::array(data_copy->size(), data_copy->data(), capsule), py_module.attr("model"));
    ...
    while (not stop_signal_called 
        ...
        {
            std::lock_guard<std::mutex> lock(mtx);
            latest_buffer.resize(num_rx_samps);
            std::memcpy(latest_buffer.data(), buffs[0].data(),
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    if (type == "double")
        ...
        throw std::runtime_error("Not supported for Numpy");
    else {
